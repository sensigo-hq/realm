      ...
      if (step['execution'] !== 'auto' || step['uses_service'] === undefined) {
        ...
        for (const [key, value] of Object.entries(step['input_map'] as Record<string, unknown>)) {
          if (typeof value === 'string') {
            // Valid dot-path — no further validation needed.
          } else if (typeof value === 'object' && value !== null) {
            const objVal = value as Record<string, unknown>;
            if (!('$literal' in objVal)) {
              errors.push(
                `Step '${stepName}': input_map['${key}'] must be a string (dot-path) or { $literal: <scalar> }`,
              );
            } else {
              const literalVal = objVal['$literal'];
              const isScalar =
                literalVal === null ||
                typeof literalVal === 'string' ||
                typeof literalVal === 'number' ||
                typeof literalVal === 'boolean';
              if (!isScalar) {
                errors.push(
                  `Step '${stepName}': input_map['${key}'] $literal value must be a scalar (string, number, boolean, or null)`,
                );
              }
            }
          } else {
