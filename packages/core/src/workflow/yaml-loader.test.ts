...
`;
  ...
  it('input_map with $literal scalar values is accepted', () => {
    const content =
      SERVICE_YAML_BASE +
      '    input_map:\n      a: { $literal: "CS_Macros" }\n      b: { $literal: true }\n      c: { $literal: 50 }\n      d: { $literal: null }\n';
  ...
  it('input_map with $literal object value is rejected', () => {
    const content = SERVICE_YAML_BASE + '    input_map:\n      x: { $literal: { nested: true } }\n';
    expect(() => loadWorkflowFromString(content)).toThrow(WorkflowError);
    try {
      loadWorkflowFromString(content);
    } catch (err) {
      expect((err as WorkflowError).message).toContain('$literal value must be a scalar');
    }
  });
  ...
  it('input_map with object missing $literal key is rejected', () => {
    const content = SERVICE_YAML_BASE + '    input_map:\n      x: { other_key: "value" }\n';
    expect(() => loadWorkflowFromString(content)).toThrow(WorkflowError);
    try {
      loadWorkflowFromString(content);
      ...
      expect((err as WorkflowError).message).toContain(
        'must be a string (dot-path) or { $literal: <scalar> }',
      );
    }
  });
});
