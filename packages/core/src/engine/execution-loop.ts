...
/**
 * Resolves a single input_map value against the run root.
 * Handles: string dot-paths, { $literal: scalar }, and nested object maps.
 */
function resolveInputMapValue(value: unknown, root: Record<string, unknown>): unknown {
  if (typeof value === 'string') {
    return resolvePath(value, root);
  }
  if (typeof value === 'object' && value !== null) {
    ...
    // Nested object — recursively resolve each entry.
    const nested: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(obj)) {
      nested[k] = resolveInputMapValue(v, root);
    }
    return nested;
  }
  return value;
...
function resolveInputMap(
  inputMap: Record<string, unknown> | undefined,
  ...
  for (const [key, value] of Object.entries(inputMap)) {
    result[key] = resolveInputMapValue(value, root);
  }
  return result;
}
