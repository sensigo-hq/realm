...
| `trust`             | `'auto'` \| `'human_confirmed'` \| `'human_reviewed'` | No       | Human oversight level. Any other value is a load error (issue #508). See [Trust levels](#trust-levels).                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- `trust` must be absent, `'auto'`, or a recognized gate literal (`human_confirmed`/`human_reviewed`) — but ONLY `'auto'` is accepted: a gate literal is rejected because **a finalizer must not gate**, and any other value is rejected separately because it is **not a recognized `trust` value at all** (see [Trust levels](#trust-levels); issue #508).
...
Any other value — a typo, the [service `trust`](#services) literal used by mistake (`engine_delivered` / `engine_managed` / `agent_provided` — a **different key**, `services.<name>.trust`, not a step's own `trust:`), the retired `human_notified` value, `null`, or any non-string — is a **load error** on `execution: auto` and `execution: agent` steps (issue #508): no gate opens, and the step never runs unattended on an unrecognized value. A definition that somehow bypasses the loader (for example, a store record edited by hand) is refused again by the engine immediately before dispatch, with the same remedy: correct the value, `realm workflow register` the corrected file, and retry.
...
| `trust`   | `'engine_delivered'` \| `'engine_managed'` \| `'agent_provided'` | Service trust level — a **different key** from a step's own `trust:` (see [Trust levels](#trust-levels)). See below. |
