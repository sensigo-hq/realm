...
count says what it could not count (`0 workflows registered; 1 entry in the registry could not be
read.`), so a stdout-only reader is never told the registry is clean when it is not. Files whose
...
| the copy exists but cannot be read       | `the registered copy of '<id>' could not be read (<errno>: <path>)`                                                                                           | make that path readable (`chmod u+r`; a permission, a mount)                                                                                                                       |
| a directory sits where the file belongs  | `<path> is a directory, not a workflow file`                                                                                                                  | remove the directory (`rm -r`), then re-register from source                                                                                                                       |
| the copy is empty or corrupt             | `… is empty (0 bytes) — not a workflow` / `… is not parseable JSON: …`                                                                                        | re-register from source; if the file was never registered from a source, remove it (`rm`) — for a registered copy, removing it repairs nothing (the next attempt says "not found") |
...
| the registry directory cannot be read    | `the workflow registry at <dir> cannot be read (<errno>)`                                                                                                     | make that directory readable and searchable (`chmod u+rx`)                                                                                                                         |
...
and its repair clause ends in the answer command: `This run is waiting on human gate '<step>',
which cannot be answered until its workflow can be read; realm run abandon refuses a run that is
waiting on a gate. To repair: make <path> readable (chmod u+r <path>), then answer the gate (realm run respond <run-id> --gate
<gate-id> --choice <one of: …>).` On a run that is already terminal the refusal says so —
...
| **Diagnostics: `~N tokens (estimate, step input)`** | A character-based approximation of THIS STEP'S RESOLVED INPUT (`JSON.stringify(input).length / 4`), computed whether or not a model call happens. It is NOT the context window and NOT the prompt the provider received — those include the system block, the output schema and the conversation, and in a real run differ from this number by ~100x. For the prompt the provider actually billed, read the measured row below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **Diagnostics: `N prompt tokens (measured, …)`**    | The whole prompt size the provider billed. When ONE wire request reported a prompt, that request's figure, with the label naming which (`measured, first request`, or `measured, request 2 of 3`). When SEVERAL reported one, their TOTAL, with the label naming how many of how many (`measured, totals across 2 of 4 requests`) — showing one request's figure while a larger sibling went unshown understated what the step billed, and the failed-drive line sums the identical data shape. A real measurement, distinct from the estimate above and stated separately from it. When the step made at least one wire request and none of them reported a prompt, the line says `prompt not reported` rather than omitting the figure (with no per-request detail at all there is no prompt segment); when the provider reported only the uncached part of the prompt, the line shows that part, labelled as the part. |
| **Diagnostics: `cache: ...`**                       | What the provider reported about prompt caching for this step: tokens read from cache, tokens written to it, and how many wire requests the counts cover. Each DIRECTION is reported independently — a direction no request reported reads `read not reported`, never `read 0`, because a `0` on this line always means a provider said zero. A direction only some requests reported reads as a floor that names what it counted (`read at least 512 (1 of 3 requests reported a read)`). `not engaged — read 0, wrote 0` means both directions were reported and both were zero. Says nothing when no model call happened; says the provider reported nothing observable when one did but gave no cache signal; and names a `state` or `basis` word this build does not know as unrecognised rather than printing it as an ordinary fact.                                                                               |
...
Resets a failed or abandoned run so a specific FAILED step can re-execute — the step named by `--from` must be in the run's `failed_steps`, so a run with none has no resume path.
...
Idempotent (abandoning an already-abandoned run succeeds as a no-op — the one terminal state it accepts). Refuses any other terminal run, and
...
Both arguments come from `realm run inspect`: the id on its `Gate:` line, the choices on the
`Choices:` line below it. Should you mistype one anyway, the refusal is safe — `Choice '<x>' is
not valid. Expected one of: …`, and nothing is recorded.
