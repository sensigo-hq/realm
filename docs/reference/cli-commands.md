...
| **Diagnostics: `N prompt tokens (measured, first request)`** | The whole prompt size the provider billed for this step's first wire request — a real measurement, distinct from the estimate above and stated separately from it. Present only when the step made a model call.                                                                                      |
| **Diagnostics: `cache: ...`**                                | What the provider reported about prompt caching for this step: tokens read from cache, tokens written to it, and how many wire requests the counts cover. Says nothing when no model call happened; says the provider reported nothing observable when one did but the provider gave no cache signal. |
