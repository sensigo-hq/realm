...
| **Diagnostics: `N prompt tokens (measured, <which> request)`** | The whole prompt size the provider billed, taken from the first wire request that REPORTED one — a prompt is not additive across requests, so this is one request's figure and the label names which (`first request`, or `request 2 of 3`). A real measurement, distinct from the estimate above and stated separately from it. When the step made a model call and no request reported a prompt the line says `prompt not reported` rather than omitting the figure; when the provider reported only the uncached part of the prompt, the line shows that part, labelled as the part.                                                                                                                                                                                                                                                     |
