...
| `diagnostics`        | array    | Structured, code-tagged counterpart to `warnings` — `{ code, severity, message, scope, step?, key?, did_you_mean? }` per entry. Additive-optional; only `create_workflow` populates it today (see below), every other tool omits it. Lets an agent branch on `code`/`key`/`did_you_mean` instead of parsing `warnings` text. |
