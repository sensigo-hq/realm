...
| `wait_and_proceed`     | The step was rate-limited by an upstream service. `retry_after` in the response gives the delay in seconds. This action appears only when the step has no engine-level `retry` config; when retry is configured, the engine retries internally. | Wait `retry_after` seconds, then follow `next_actions` without human involvement. The step has already failed — do not call `execute_step` for the same command again; follow `next_actions` instead. |
